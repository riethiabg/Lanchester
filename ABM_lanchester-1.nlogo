...
to go-forces
  ask turtles [ turtle-attack ]
end

to turtle-attack
  let target one-of turtles with [color != [color] of myself]
  if target != nobody [ ask target [ die ] ]
end
...
Lanchester (1916) presented a now classic deterministic model of outcomes of ranged combat based on the fighting effectiveness and the troop strengths (i.e., number of soldiers) of two opposing forces. The system of differential equations that came to be known as the Lanchester Square Law is given by
...
To setup, all variables and agents are cleared, and then the square space is filled with blue agent and red agents. To go, in random order, each agent kills exactly one enemy, if an enemy remains to kill. The two forces are equally effective. 
