...
to setup-forces
  create-turtles initial-blue-forces [
    set color blue
    setxy random-xcor random-ycor
  ]
  create-turtles initial-red-forces [
    set color red
    setxy random-xcor random-ycor
  ]
end
...
to turtle-attack
  let targets turtles with [color != [color] of myself]
  let target min-one-of targets [distance myself]
  if target != nobody [
    let accuracy 0
    ifelse color = blue [ set accuracy blue-accuracy ] [ set accuracy red-accuracy ]
    if random-float 1 < (accuracy ^ (distance target)) [
      ask target [ die ]
    ]
  ]
end
